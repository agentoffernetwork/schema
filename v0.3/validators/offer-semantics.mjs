...
function validateOfferCommonV03Semantics(offer, errors) {
...
export function validatePublicOfferV03Semantics(offer) {
  const rootError = validateOfferRoot(offer)
  if (rootError) return { valid: false, errors: [rootError] }
  const errors = []
  validateOfferCommonV03Semantics(offer, errors)
...
export function validatePartnerOfferV03Semantics(offer) {
  const rootError = validateOfferRoot(offer)
  if (rootError) return { valid: false, errors: [rootError] }
  const errors = []
  validateOfferCommonV03Semantics(offer, errors)
...
export function validateOfferQueryResponseV03Semantics(response, request = {}) {
  ...
  if (response.language !== undefined && (typeof response.language !== "string" || !BCP_47_LANGUAGE_TAG.test(response.language) || !hasUniqueLanguageExtensionSingletons(response.language))) errors.push("language must use the final-v0.3 language-tag profile")
