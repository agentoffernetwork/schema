...
const goalEventSchema = JSON.parse(readFileSync(resolve(here, '../json-schema/goal-event-name-v0.3.json'), 'utf8'));
...
export function validatePostbackV03Semantics(postback, context) {
  const errors = [];
  ...
  if (!Array.isArray(declaredGoals) || declaredGoals.length === 0 || declaredGoals.some((name) => typeof name !== 'string' || !goalEventPattern.test(name)) || new Set(declaredGoals).size !== declaredGoals.length) {
    errors.push(error('context_invalid', '/context/declared_goal_events', 'declared_goal_events must be a unique non-empty v0.3 goal list'));
...
export default validatePostbackV03Semantics;
